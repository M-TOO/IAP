<div class="admin-layout">
    <style>
        /* --- RESET & LAYOUT --- */
        .content-box-small { max-width: 100% !important; width: 100% !important; margin: 0 !important; padding: 0 !important; background: transparent !important; border: none !important; box-shadow: none !important; }
        ...
        .nav-link.active { background: var(--color-vendor); color: #fff; font-weight: bold; } /* Green for Vendors */
    ...
    <aside class="admin-sidebar" id="sidebar">
        ...
        <div style="flex: 1;">
            ...
            <a href="{{ route('admin.vendors') }}" class="nav-link active" title="Manage Vendors">
    ...
    <main class="admin-main" id="mainContent">
        <h2 style="color: white; font-size: 2rem; margin-bottom: 30px; text-shadow: 0 2px 4px rgba(0,0,0,0.5);">Vendor Management</h2>

        @if($statusMessage) <div class="glass-panel" style="background: rgba(76, 175, 80, 0.2); border-color: #4CAF50; color: #4CAF50; text-align: center;">{{ $statusMessage }}</div> @endif
        ...
        @if($pendingVendors->isNotEmpty())
            <div class="glass-panel" style="border-left: 5px solid #e67e22;">
                <h3 style="color: #e67e22; margin-top: 0;">Pending Approvals ({{ $pendingVendors->count() }})</h3>
                <table class="admin-table">
                    <thead><tr><th>Owner</th><th>Shop</th><th>ID</th><th>Applied</th><th>Actions</th></tr></thead>
                    <tbody>
                        @foreach($pendingVendors as $vendor)
                        <tr>
                            <td style="font-weight: bold; color: white;">{{ $vendor->owner_name }}</td>
                            <td>{{ Str::limit($vendor->shop_description, 30) }}</td>
                            <td>{{ $vendor->national_id }}</td>
                            <td>{{ $vendor->created_at->diffForHumans() }}</td>
                            <td>
                                <button wire:click="approve({{ $vendor->id }})" style="background: var(--color-vendor); color: white; border: none; padding: 6px 15px; border-radius: 4px; cursor: pointer; margin-right: 5px;">Approve</button>
                                <button wire:click="reject({{ $vendor->id }})" style="background: #d32f2f; color: white; border: none; padding: 6px 15px; border-radius: 4px; cursor: pointer;">Reject</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="glass-panel">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3 style="color: var(--color-vendor); margin: 0;">Active Database</h3>
                <input wire:model.live="search" type="text" placeholder="Search vendors..." style="background: rgba(0,0,0,0.3); border: 1px solid #444; padding: 8px 15px; border-radius: 6px; color: white; width: 250px;">
            </div>
            <table class="admin-table">
                <thead><tr><th>Owner</th><th>Shop</th><th>Email</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $vendor->email }}</td>
                        <td>
                            @if($vendor->status === 'suspended') <span class="badge badge-suspended">Suspended</span>
                            @else <span class="badge badge-active">Active</span> @endif
                        </td>
                        <td>
                            @if($vendor->status === 'suspended')
                                <button wire:click="unsuspendVendor({{ $vendor->id }})" style="background: transparent; border: 1px solid #4CAF50; color: #4CAF50; padding: 4px 10px; border-radius: 4px; cursor: pointer;">Unsuspend</button>
                            @else
                                <button wire:click="confirmSuspension({{ $vendor->id }})" style="background: transparent; border: 1px solid #ff9800; color: #ff9800; padding: 4px 10px; border-radius: 4px; cursor: pointer;">Suspend</button>
                            @endif
                        </td>
                    </tr>
                    @empty <tr><td colspan="5" style="text-align: center; padding: 30px; color: #888;">No vendors found.</td></tr>
                    @endforelse
                </tbody>
            </table>
            <div style="margin-top: 20px;">{{ $approvedVendors->links(data: ['scrollTo' => false]) }}</div>
        </div>
    </main>
    ...
    @if($confirmingSuspension)
        <div class="modal-overlay">
            <div class="modal-content">
                <h3 style="color: var(--color-vendor);">Suspend Vendor</h3>
                <p style="color: #ccc;">Select duration for <strong>{{ $vendorToSuspendName }}</strong></p>
                ...
                <div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px;">
                    <button wire:click="suspendVendor" style="background: #d32f2f; color: white; border: none; padding: 10px 20px; border-radius: 4px; cursor: pointer;">Confirm</button>
